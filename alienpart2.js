document.addEventListener("DOMContentLoaded", function () {
    ...
    var instaCitizenshipButton = document.getElementById("instaCitizenship"); // Fixed ID issue
    ...
    alienForm.addEventListener("submit", function (event) {
        ...
        var captcha = document.getElementById("captcha").value; // Fixed incorrect ID
        var selectedPowers = document.querySelectorAll("input[name='powers']:checked");
        if (alienName.length < 5) {
            alert("Alien name is too Human! Try an Alien Name!");
            return;
        }
        if (alienAge < 50) {
    ...
    instaCitizenshipButton === null || instaCitizenshipButton === void 0 ? void 0 : instaCitizenshipButton.addEventListener("click", function () {
        alert("Ha! Ha! Nice Try, Earthling! You think Insta Citizenship is that Easy??");
    });
